...
The shared flow should combine:

* deterministic Kotlin validation and policy steps
* LLM-driven tool-calling steps where Koog mediates tool access
...
2. use the LLM to call Wikipedia search tools in a restricted search stage
3. select candidate articles deterministically from the tool results
4. use the LLM to call article fetch tools in a restricted fetch stage
5. evaluate evidence sufficiency deterministically
6. synthesize study notes and quiz as structured output
...
* The LLM must **not** have unrestricted access to all tools for the whole workflow.
* Search-stage LLM access should be limited to search tools only.
* Fetch-stage LLM access should be limited to article fetch tools only.
* Validation, article selection, and evidence sufficiency should remain deterministic Kotlin logic.
...
The intended vNext refactor is:

* the LLM requests search and fetch tools through Koog tool-calling nodes
* Kotlin still owns ranking, deduplication, and evidence policy
* the final structured screen payload is still generated by the LLM from retrieved evidence
